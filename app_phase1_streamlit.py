...
def cover_sizes_cleanup(root):
    ...
    for i,txt in enumerate(texts):
        low=txt.lower()
        if "fiche de cours" in low: set_size(paras[i], 22); in_plan=True; continue
        ...
        if txt.strip().upper()=="PLAN": set_size(paras[i], 11); in_plan=True; continue
        if in_plan:
            # toutes les lignes du plan en taille 11
...
def remove_large_grey_rectangles(root, theme_colors):
    ...
    for drawing in root.findall(".//w:drawing", NS):
        ...
        for sc in holder.findall(".//a:schemeClr", NS):
            ...
            r=min(255,int(r*mod+255*off)); g=min(255,int(g*mod+255*off)); b=min(255,int(b*mod+255*off))
            srgb.append(f"{r:02X}{g:02X}{b:02X}")
        looks_f2=any(is_close_grey(v,"#F2F2F2",16) for v in srgb)
        is_rect=holder.find(".//a:prstGeom[@prst='rect']", NS) is not None or holder.find(".//a:prstGeom[@prst='roundRect']", NS) is not None
...
def reposition_small_icon(root, left_cm=15.3, top_cm=11.0):
    # sélectionne l’image la plus à droite dans la moitié haute de la page
    cand=[]
    for anchor in root.findall(".//wp:anchor", NS):
        ...
        if anchor.find(".//pic:pic", NS) is None: continue
        if cx>cm_to_emu(3.0) or cy>cm_to_emu(3.0): continue
        ...
        try: x_cm=emu_to_cm(int(x)); y_cm=emu_to_cm(int(y))
        except: x_cm=y_cm=0.0
...
# --- Ajustement des tailles DML sur la couverture ---

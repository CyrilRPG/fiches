...
def strip_actualisation_everywhere(root):
    # Supprimer “ACTUALISATION” dans tous les w:t et a:t
...
def cover_sizes_cleanup(root):
    """
    Ajuster les tailles du texte de la page de garde :
    - ‘Fiche de cours …’ → 22 pt
    - Nom du cours (ligne immédiatement après) → 20 pt
    - Encadré Université + année → 10 pt
    - Tout le plan → 11 pt
    - Supprimer le mot ACTUALISATION
    """
    ...
    for i, txt in enumerate(texts):
        ...
        # Ligne après “Fiche de cours …” = nom du cours
        if last_was_fiche and txt:
            set_size(paras[i], 20)
            last_was_fiche = False

        # Encadré université + année
        ...
        # Tous les éléments du plan (jusqu’à un paragraphe vide ou Légende)
        if in_plan:
        ...
        # Supprimer “ACTUALISATION”
        if txt.strip().upper() == "ACTUALISATION":
            for t in paras[i].findall(".//w:t", NS):
                if t.text:
                    t.text = re.sub(r"(?i)ACTUALISATION", "", t.text)

# --- Tables et numérotation ---
def tables_and_numbering(root):
    # Titre du tableau = 12 pt gras ; reste 9 pt
    ...
    # Numérotation romaine → 10 pt, italique, gras, blanc
    for p in root.findall(".//w:p", NS):
...
def looks_like_cover_shape(holder):
    ...
    if posV is None:
        return True
    try:
        return emu_to_cm(int(posV.text)) < 20.0
    except:
        return True

def remove_large_grey_rectangles(root, theme_colors):
    ...
    for p in root.findall(".//w:p", NS):
        if get_text(p).strip().lower() == "légendes":
            for t in p.findall(".//w:t", NS):
                t.text = ""
    lines = {"Notion nouvelle cette année", "Notion hors programme",
             "Notion déjà tombée au concours", "Astuces et méthodes"}
    for p in root.findall(".//w:p", NS):
...
def insert_legend_image(document_xml: bytes, rels_xml: bytes, image_bytes: bytes,
    ...
    (ET.SubElement(paras[idx], f"{{{W}}}r") if idx is not None else
     ET.SubElement(ET.SubElement(root, f"{{{W}}}p"), f"{{{W}}}r")).append(drawing)
    return (ET.tostring(root, encoding="utf-8", xml_declaration=True),
            ET.tostring(rels, encoding="utf-8", xml_declaration=True),
            (f"word/{media_name}", image_bytes))

# --- Repositionnement de l’icône écriture ---
def reposition_small_icon(root, left_cm=15.3, top_cm=11.0):
    ...
    if not cand:
        return
    # Choisir l’icône la plus à droite dans la partie haute (6–16 cm)
    zone = [c for c in cand if 6.0 <= c[1] <= 16.0]
    chosen = max(zone or cand, key=lambda t: t[0])
    anchor = chosen[2]
    # repositionner
...
# --- Tailles des formes DML sur la couverture ---
def tune_cover_dml_textsizes(root):
    """
    Ajuster les tailles des formes DML de couverture :
    - Université + année → 10 pt
    - ‘Fiche de cours …’ → 22 pt
    - ‘Introduction … Biologie’ (nom du cours) → 20 pt
    - Supprimer ‘ACTUALISATION’ s’il apparaît dans une forme
    """
...
def process_bytes(docx_bytes: bytes,
    ...
    # gestion de la légende (facultative)
    ...
    if not files:
        ...
        for up in files:
            try:
                ...
                # Retirer le mot 'ACTU' du nom d’origine (insensible à la casse)
                orig_name = up.name
                out_name = re.sub(r"(?i)\s*ACTU", "", orig_name)

...
def force_footer_size_10(root):
    """
    Force la taille du pied de page à 10 pt tout en préservant les champs
    automatiques (numéro de page).  Les exécutions contenant des éléments
    de champ (w:fldChar) ou des instructions (w:instrText) ne sont pas
    modifiées pour éviter de corrompre la numérotation.
    """
    for r in root.findall(".//w:r", NS):
...
def extract_theme_colors(parts: Dict[str, bytes]) -> Dict[str, str]:
    """
    Extrait les couleurs du thème principal (theme1.xml) pour décoder les
    couleurs déclarées via schemeClr.
    """
...
def remove_large_grey_rectangles(root: ET.Element, theme_colors: Dict[str, str]):
    ...
    - Seules les formes rectangulaires sans image sont candidates à la suppression.
    - Les couleurs explicites (srgbClr) sont utilisées pour détecter un gris
      clair proche de F2F2F2.  Les formes utilisant uniquement des couleurs
      de thème (schemeClr) ne sont plus supprimées afin de ne pas toucher
      au design.
    - Un rectangle très grand situé sur la moitié droite (≥7×12 cm, x≥9 cm)
      est également supprimé même sans couleur explicite.  Cela permet de
      retirer l’encart gris de la page de garde qui masque le logo.
    """
    ...
    for drawing in root.findall(".//w:drawing", NS):
        ...
        # calcul précis des positions via posOffset
        ...
        # couleurs explicites déclarées en srgbClr
